...
curr_score_track = currentTimeline.GetTrackName("video", 3)
sorted_marker_keys = sorted(markersList.keys())

cyan_markers = [
    m for m in sorted_marker_keys if markersList[m]['color'] == 'Cyan']


keys_file_names = [(k, '%s.png' %
                    markersList[k]['name']) for k in cyan_markers]
